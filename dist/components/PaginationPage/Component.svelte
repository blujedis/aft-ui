...
$:
  paginationPageClasses = th.create("PaginationPage").variant("paginationPage", variant, theme, true).option("focusedRingVisible", theme, focused).option("fieldFontSizes", size, size).option("paginationGroupedPadding", size, size && variant === "filled").option("paginationFlushedPadding", size, size && variant === "flushed").option("roundeds", rounded, rounded && (previous || next)).append(
    "relative inline-flex items-center justify-center font-semibold focus:z-20",
    variant === "filled"
  ).append("z-10", variant === "filled" && selected).append("pointer-events-none", value === "...").append("px-2", previous || next).append("rounded-r-none", previous).append("rounded-l-none", next).append($$restProps.class, true).compile(true);
$:

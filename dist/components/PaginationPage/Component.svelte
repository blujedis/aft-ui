<script>
	...
	$: paginationPageClasses = th
		...
		.option('roundeds', rounded, rounded && (previous || next))
		.append(
		...
		.append('px-2', previous || next)
		.append('rounded-r-none', previous)
		.append('rounded-l-none', next)
		.append($$restProps.class, true)
		.compile(true);
	$: paginationPageIconClasses = th
		.create('PaginationPageIcon')
		.option('paginationIconSizes', size, size && (previous || next))
		.compile(true);

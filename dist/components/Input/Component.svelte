...
$:
  inputClasses = unstyled ? th.create("Input").append($$restProps.class, true).compile(true) : th.create("Input").variant("input", variant, theme, true).option("focused", theme, focused).option("focusedRingSizes", "two", focused).option("focusedBorderFlushSizes", "two", focused && variant === "flushed").remove("focusedFilters", focused, focused).option("placeholders", theme, true).option("common", "transition", transitioned).option("fieldFontSizes", size, size).option("fieldPadding", size, size).option("roundeds", rounded, rounded && variant !== "flushed").option("shadows", shadowed, shadowed).option("disableds", theme, disabled).append("w-full", full).append("px-2", variant === "flushed").append("flex items-center justify-center", true).append($$restProps.class, true).compile(true);

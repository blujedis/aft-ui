...
$:
  popoverClasses = th.create("Popover").variant("globals", variant, theme, variant).option("roundeds", rounded, rounded).option("shadows", shadowed, shadowed).option("common", "transition", transitioned).option("fieldFontSizes", size, size).option("popoverSizes", size, size).append("absolute animate-fade-in-down", true).append($$restProps.class, true).compile(true);
</script>

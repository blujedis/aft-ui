...
$:
  selected = $context.selected.map(
    (v) => $context.items.find((item) => v === item.value)
  );
$:
...
$:
  containerClasses = th.create("SelectListButton").variant("selectListButton", variant, theme, variant).option("focusedRing", theme, focused && variant !== "flushed").option("common", "transitioned", transitioned).option("fieldFontSizes", size, size).option("roundeds", rounded, rounded && variant !== "flushed").option("shadows", shadowed, shadowed).option("common", "disabled", disabled).append("w-full", full).append("inline-flex items-center justify-between relative min-w-[176px] peer", true).append("outline-none focus-visible:ring-2", focused && variant !== "flushed").append("border-0", variant === "flushed").append($$restProps.class, true).compile(true);
$:
...
$:
  inputClasses = th.create("SelectListInput").option("fieldFontSizes", size, size).option("fieldPadding", size, size).append("bg-transparent outline-none border-none", true).append("invisible", disabled).append("w-12 ml-1 pl-0", tags).append("caret-transparent truncate cursor-pointer", !tags).append("min-w-min", tags && !selected.length).append("group peer", true).compile(true);
$:
  buttonClasses = th.create("SelectListButtonIconWrapper").option("fieldPaddingX", "md", "md").option("focusedRingVisible", theme, focused).append("pt-px pl-0 outline-none focus-visible:ring-2", true).append("pl-0", tags).compile(true);
$:
  tagClasses = th.create("SelectListButtonTag").option("focusedRingVisible", theme, focused).append("mr-0.5 outline-none focus-visible:ring-2", true).compile(true);
$:

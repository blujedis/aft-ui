...
$:
  paginationDetailClasses = themer($themeStore).create("PaginationDetail").variant("paginationDetail", variant, theme, true).option("common", "transition", transitioned).remove(transitioned === "colors" ? "transition-all" : "transition-colors", transitioned).option("badgePadding", size, size).option("badgeFontSizes", size, size).option("roundeds", rounded, rounded).option("shadows", shadowed, shadowed).append("w-full", full).append("flex items-center justify-center", true).append($$restProps.class, true).compile(true);
</script>

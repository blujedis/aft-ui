...
$:
  checkboxClasses = themer($themeStore).create("Checkbox").variant("checkbox", variant, theme, true).option("focusedRing", theme, focused).option("common", "transition", transitioned).option("checkboxSizes", size, size).option("roundeds", rounded, rounded).option("shadows", shadowed, shadowed).option("disableds", theme, disabled).append("w-full", full).append(

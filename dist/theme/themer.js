...
export function themer(themeConfig) {
	...
	function create(instanceName = '') {
		...
		function option(key, prop, when) {
			...
			if (!opt) throw new Error(`${instanceName} option using property ${prop} was NOT found.`);
		...
		function mapped(obj, key, when) {
			...
			if (typeof value !== 'string')
				throw new Error(
					`${instanceName} mapped value using property ${key} has invalid typeof ${typeof value}.`
				);

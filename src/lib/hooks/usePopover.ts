...
function hasMiddleware(middleware: PopoverComputeMiddleware | PopoverComputeMiddleware[], ...keys: string[]) {
...
export function createPopoverInstance(instanceOptions?: PopoverOptions) {
  ...
  options.middleware =
    middlewareDefaults(options.middleware, { offset: options.offset, padding: options.padding })
  ...
  function registerTrigger(
    node: string | HTMLElement | VirtualElement | Readable<VirtualElement>,
  ) {
    const foundNode = typeof node === 'string' ? findElement(node, containerNode) : node;
    if (!foundNode) throw new Error(`Cannot create Popover using trigger element of null or undefined.`);
  ...
  function registerContent(
    node: HTMLElement,
    registerOptions = {} as PopoverContentOptions
  ) {

export const selectOption = {
	default: {
		$base: 'text-inherit border',
		dark: 'text-white border-white hover:border-frame-100',
		light: 'text-frame-600 border-frame-200 hover:border-frame-300',
		primary: 'text-primary-600 border-primary-500 hover:border-primary-600',
		secondary: 'text-secondary-600 border-secondary-500 hover:border-secondary-600',
		tertiary: 'text-tertiary-600 border-tertiary-500 hover:border-tertiary-600',
		danger: 'text-danger-600 border-danger-500 hover:border-danger-600',
		warning: 'text-warning-600 border-warning-500 hover:border-warning-600',
		success: 'text-success-600 border-success-500 hover:border-success-600',
		info: 'text-info-600 border-info-500 hover:border-info-600'
	},
	outlined: {
		$base: 'text-inherit border',
		dark: 'text-white border-white hover:border-frame-100',
		light: 'text-frame-600 border-frame-200 hover:border-frame-300',
		primary: 'text-primary-600 border-primary-500 hover:border-primary-600',
		secondary: 'text-secondary-600 border-secondary-500 hover:border-secondary-600',
		tertiary: 'text-tertiary-600 border-tertiary-500 hover:border-tertiary-600',
		danger: 'text-danger-600 border-danger-500 hover:border-danger-600',
		warning: 'text-warning-600 border-warning-500 hover:border-warning-600',
		success: 'text-success-600 border-success-500 hover:border-success-600',
		info: 'text-info-600 border-info-500 hover:border-info-600'
	},
	filled: {
		$base: 'text-inherit',
		dark: 'text-white bg-white hover:bg-frame-200',
		light: 'text-frame-600 bg-frame-50 hover:bg-frame-200',
		primary: 'text-primary-600 bg-primary-50 hover:bg-primary-200',
		secondary: 'text-secondary-600 bg-secondary-50 hover:bg-secondary-200',
		tertiary: 'text-tertiary-600 bg-tertiary-50 hover:bg-tertiary-200',
		danger: 'text-danger-600 bg-danger-50 hover:bg-danger-200',
		warning: 'text-warning-600 bg-warning-50 hover:bg-warning-200',
		success: 'text-success-600 bg-success-50 hover:bg-success-200',
		info: 'text-info-600 bg-info-50 hover:bg-info-200'
	},
	flushed: {
		$base:
			'text-inherit border border-l-0 border-r-0 border-t-0 focus-visible:!ring-0 focus-visible:border-b-2',
		dark: 'text-white border-white focus:border-frame-200',
		light: 'text-frame-600 border-frame-200 focus:border-frame-500',
		primary: 'text-primary-600 border-primary-500 focus:border-primary-600',
		secondary: 'text-secondary-600 border-secondary-500 focus:border-secondary-600',
		tertiary: 'text-tertiary-600 border-tertiary-500 focus:border-tertiary-600',
		danger: 'text-danger-600 border-danger-500 focus:border-danger-600',
		warning: 'text-warning-600 border-warning-500 focus:border-warning-600',
		success: 'text-success-600 border-success-500 focus:border-success-600',
		info: 'text-info-600 border-info-500 focus:border-info-600'
	}
};

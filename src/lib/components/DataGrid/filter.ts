...
import { isEmpty, isEqual, isGreaterThan, isGreaterThanOrEqual, isLessThan, isLessThanOrEqual, isNotEqual, isNotEmpty, isLike } from '$lib/utils/is';
  ...
  const result = rows.filter(r => {
    ...
    if (ands.length)
      hasAnds = ands.every(c => {
        ...
        return handler((col as Required<DataGridColumnConfig<D>>).transform(c.query), r[c.accessor]);

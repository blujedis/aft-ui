<script lang="ts">
	...
	type OptionTypes = Omit<TypedDefaults<typeof component>, 'base' | 'variant' | 'theme'>;

	const title = 'Labels';
	const description = 'Themed labels.';

	const { palette, components } = $themeStore;
	const component = withThemes(components.label.main, palette);
...
<Page {title} {description}>
	<div class="-m-2 flex space-x-4">
		<!-- <label class="text-sm inline-flex items-center dark:text-gray-300 m-2">
			<input type="checkbox" bind:checked={options.pointer} class="dark:bg-transparent"/> 
			<span class="pl-2">Pointer</span>
		</label> -->
